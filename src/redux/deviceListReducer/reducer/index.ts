...
export const _reducer = (state: _reducer_i = initialState, action: _actionReturnTypes) => {
  switch (action.type) {
    case _reduxConstant.DEVICELIST_REDUX:
      ...
      return Object.assign({}, state, {
        deviceList: action.props.deviceList.length ? [...action.props.deviceList] : [],
      });
      break;

    case _reduxConstant.DELETED_DEVICELIST_REDUX:
      action.props.log?.print(
        'updating DELETED DeviceList to store >> ' + JSON.stringify(action.props.deletedDeviceList)
      );
      if (!action.props.isDbState) storeData('deletedDeviceList', action.props.deletedDeviceList);
      return Object.assign({}, state, {
        deletedDevices: action.props.deletedDeviceList.length ? [...action.props.deletedDeviceList] : [],
      });
      break;
