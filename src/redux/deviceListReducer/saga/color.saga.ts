...
const getHex: (props: getHex_i) => [string | undefined, UNIVERSALS.GLOBALS.DEVICE_t] = ({ channelBrightnessObject, stateObject, ...props }) => {
  ...
  if (channelBrightnessObject) {
    ...
    if (channelBrightnessObject.value < 10) {
      if (newDevice.channel.preState != UNIVERSALS.GLOBALS.channelState_e.CH_STATE_OFF) newDevice.channel.preState = newDevice.channel.state;
    ...
    newDevice.channel.outputChannnel.forEach((channel, index) => {
      if (channel.type == UNIVERSALS.GLOBALS.outputChannelTypes_e.colorChannel_temprature) {
        ...
        if (channelBrightnessObject.activeChannel[index]) newDevice.channel.outputChannnel[index].v = channelBrightnessObject.value < 10 ? 0 : channelBrightnessObject.value;
        /// append newState with hex for this perticular channel
        if (newDevice.channel.outputChannnel[index].v <= 15) newState += '0' + newDevice.channel.outputChannnel[index].v.toString(16);
        else if (newDevice.channel.outputChannnel[index].v < 5) newState += '00' + newDevice.channel.outputChannnel[index].v.toString(16);
    ...
    // Handles `CH_STATE_RGB` stateObject for RGB device channel
      ...
      if (stateObject?.hsv?.h) newDevice.channel.outputChannnel[0].h = stateObject?.hsv?.h;
