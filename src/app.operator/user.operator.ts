...
interface loginFunction_props {
  ...
  email: string;
  password: string;
  onLoginSuccess?: (user: USER_t) => void;
  onValidateDataFailed?: (props: API.baseError<any, loginValidationError_e>) => void;
  onLoginFailed?: (
    props:
      | Pick<API.cloudAPI.user.loginAPI._loginAPi_returnType, 'ERR'>
      | { ERR?: API.baseError<any, loginValidationError_e> }
  ) => void;
  log?: logger;
}
...
const loginFunction: loginFunction_t = async ({
  ...
  API.cloudAPI.user.loginAPI.useLogin({
    email,
    password,
    onValidateDataFailed: (err) => {
      if (onValidateDataFailed) onValidateDataFailed(err);
    },
    onLoginFailed: (err) => {
      if (onLoginFailed) onLoginFailed(err);
    },
    onLoginSuccess: (res) => {
      if (res.user.id) {
        ...
        onLoginFailed
          ? onLoginFailed({
              ERR: {
                errCode: API.cloudAPI.user.loginAPI.loginApiErrors_e.LOGIN_API_UNHANDLED,
                errMsg: 'UNKNOWN RESPONSE FROM SERVER',
              },
            })
          : {};
      }
...
interface signupFunction_props {
  ...
  password: string;
  userName: string;
  onSignupSuccess?: (user: USER_t) => void;
  onValidateDataFailed?: (props: API.baseError<any, signupValidationError_e>) => void;
  onSignupFailed?: (
    props:
      | Pick<API.cloudAPI.user.signupAPI._signupAPi_returnType, 'ERR'>
      | { ERR?: API.baseError<any, signupValidationError_e> }
  ) => void;
  log?: logger;
}
...
interface fbLogin_props {
  cmd: 'FB_LOGIN';
  onFbLoginSuccess?: (user: USER_t) => void;
  onValidateDataFailed?: (props: API.baseError<any, FBLoginValidationError_e>) => void;
  onFbLoginFailed?: (
    props:
      | Pick<API.cloudAPI.user.signupAPI._signupAPi_returnType | API.cloudAPI.user.loginAPI._loginAPi_returnType, 'ERR'>
      | { ERR?: API.baseError<any, FBLoginValidationError_e> }
  ) => void;
  log?: logger;
}
type fbLoginFunction_t = (
  props: fbLogin_props
) => Promise<API.cloudAPI.user.loginAPI._loginAPi_returnType | API.cloudAPI.user.signupAPI._signupAPi_returnType>;
  ...
  try {
    ...
    Alert.alert(
      ...
      'you can try logging in with ' +
        (UNIVERSALS.venderConf.venderPrefix == 'HUE' ? 'HUElite' : UNIVERSALS.venderConf.venderPrefix) +
        ' ID'
    );
  }
  return {};
...
interface updateFunction_props {
  ...
  password: string;
  userName: string;
  onUpdateSuccess?: (user: USER_t) => void;
  onValidateDataFailed?: (props: API.baseError<any, updateValidationError_e>) => void;
  onUpdateFailed?: (
    props:
      | Pick<API.cloudAPI.user.userUpdateAPI._userUpdate_returnType, 'ERR'>
      | { ERR?: API.baseError<any, updateValidationError_e> }
  ) => void;
  log?: logger;
}
type updateFunction_t = (
  props: updateFunction_props
) => Promise<API.cloudAPI.user.userUpdateAPI._userUpdate_returnType>;
...
type containerListOperation_t = (
  props: logoutFunction_props | loginFunction_props | signupFunction_props | updateFunction_props | fbLogin_props
) => Promise<{ RES?: USER_t; ERR?: API.baseError<any, any> }>;
