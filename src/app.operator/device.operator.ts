...
interface add_updateDevices_t {
  (props: add_updateDevices_props): UNIVERSALS.GLOBALS.DEVICE_t[];
}
const add_updateDevices: add_updateDevices_t = ({ newDevices, cloudState, ...props }) => {
  ...
  newDevices.forEach((newDevice, newDevice_index) => {
    ...
    if (localStateDevice) {
      ...
      beforeUpdateDeviceSideEffect({
        device: newDevice,
        preState: localStateDevice,
      });
      if (
        localStateDevice.localTimeStamp < newDevice.localTimeStamp ||
        ...
        (localStateDevice.ts && newDevice.ts && localStateDevice.ts != newDevice.ts) ||
      ...
      let deviceMatchFromDeletedDeviceList = reduxStore.store
        .getState()
        .deviceReducer.deletedDevices.find((item) => item.Mac == newDevice.Mac);
      ...
      if (
        deviceMatchFromDeletedDeviceList &&
        deviceMatchFromDeletedDeviceList.localTimeStamp >= newDevice.localTimeStamp
      ) {
        // IMP - only filter deletedDeviceList in case if its a cloudState comparision, generally user could be re-pairing the device after delete
        /**
         ...
         */
        props.log?.print(
          'not adding device ' + newDevice.Mac + ' to device list as it is deleted and not updated to cloud yet'
        );
      } else {
        requireReduxUpdate = true;
        ...
        reduxStore.store.dispatch(
          reduxStore.actions.deviceList.deletedDeviceListRedux({
            deletedDeviceList: reduxStore.store
              .getState()
              .deviceReducer.deletedDevices.filter((item) => item.Mac == newDevice.Mac),
          })
        );
        beforeUpdateDeviceSideEffect({
          device: newDevice,
      ...
      (item) =>
        ...
        (() => {
          ...
          newDevices.forEach((element) => {
            if (element.Mac == item.Mac) devicesFoundInCloudState = true;
          });
          if (devicesFoundInCloudState) return true;
...
interface updateDevices_props {
  cmd: 'UPDATE_DEVICE';
  newDevice: UNIVERSALS.GLOBALS.DEVICE_t;
  postUpdate?: (props: {
    newDeviceList: UNIVERSALS.GLOBALS.DEVICE_t[];
    updatedDevice: UNIVERSALS.GLOBALS.DEVICE_t;
    preDeviceState: UNIVERSALS.GLOBALS.DEVICE_t;
  }) => void;
  log?: logger;
}
interface updateDevices_t {
  (props: updateDevices_props): UNIVERSALS.GLOBALS.DEVICE_t[];
}
const updateDevices: updateDevices_t = ({ newDevice, ...props }) => {
  let requireReduxUpdate = false;
  let localDeviceList = reduxStore.store.getState().deviceReducer.deviceList;
  let newDeviceList = localDeviceList;
  newDeviceList.forEach((item, index) => {
    ...
    if (localStateDevice) {
      ...
      requireReduxUpdate = true;
      newDeviceList = newDeviceList.filter((item) => item.Mac != newDevice.Mac);
      newDeviceList.splice(localStateDeviceIndex, 0, {
        ...newDevice,
        localTimeStamp: getCurrentTimeStampInSeconds(),
      });
    } else {
      props.log?.print('device not found in local state');
      // - [ ] add device as newDevice
    }
  });
...
const removeDevice = (props: removeDeviceProps) => {
  ...
  if (device) {
    ...
    if (device.id) {
      /** if device is synced with cloud add device in the deleted deviceList so as to remove it from the cloud later from background service */
      let __foo = reduxStore.store.getState().deviceReducer.deletedDevices;
      let deviceFromDeletedDeviceList = reduxStore.store
        .getState()
        .deviceReducer.deletedDevices.find((item) => item.Mac == props.Mac);
      if (deviceFromDeletedDeviceList) __foo = __foo.filter((item) => item.Mac != props.Mac);
      __foo.push({
        ...device,
        localTimeStamp: getCurrentTimeStampInSeconds(),
      });
...
type containerListOperation_t = (
  props: add_updateDevices_props | removeDeviceProps | colorUpdate_props | updateDevices_props
) => void;
