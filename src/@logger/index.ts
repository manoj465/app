...
export class logger {
  ...
  _print = (s: string, type?: printType) => {
    if (this.canPrint()) console.log(' ' + s + ' ');
    return this;
  };

  print = (s: string, type?: printType) => {
    if (this.canPrint()) console.log(' ' + this.prefix + ' >> ' + s + ' ');
    return this;
  };

  printPretty = (s: string) => {
    if (this.canPrint()) this.print(JSON.stringify(s, null, 2));
    return this;
  };
  ...
  _printPretty = (s: object | undefined) => {
    if (this.canPrint() && s) console.log(' ' + JSON.stringify(s, null, 2) + ' ');
    return this;
  };

  /** @deprecated */
  printDeviceList = (deviceList?: DEVICE_t[]) => {
    /*   if (!deviceList)
  ...
  canPrint = () => {
    if (this.printAll) return true;
    let cprint = false;
    this.printable.forEach((item) => {
      if (this.prefix.includes(item.toUpperCase())) cprint = true;
    });
    return cprint;
  };
}
