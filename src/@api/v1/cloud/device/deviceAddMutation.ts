...
export const v1: (props: fun_t) => Promise<_createDeviceMutationAPi_returnType> = async ({
  device,
  userID,
  log,
}: fun_t) => {
  interface variables_i extends Device_t {
    userID: string;
  }
  var res = await makeHttpQuery<response_i, error_i, variables_i, _createDeviceMutationAPi_returnType>({
    ...
    resolveData: (props) => {
      ...
      if (props.ERR?.error) {
        ...
        if (
          props.ERR.error.message == 'E11000 duplicate key error collection: huelite.hue_devices index: Mac_1 dup key:'
        ) {
          return {
            ERR: {
              errCode: createdeviceMutationApiErrors_e.CREATE_DEVICE_MUTATION_API_DUPLICATE_MAC_ERROR,
