...
interface response_i {
  updateUser: Omit<User_t, 'devices'>;
}
...
export const v1: (props: fun_t) => Promise<_userUpdate_returnType> = async ({ id, userName, password, log }: fun_t) => {
  ...
  if (password) _var = { ..._var, password };
  if (userName) _var = { ..._var, userName };
    ...
    resolveData: (props) => {
      ...
      if (props.ERR) {
        ...
        if (
          ...
          return {
            ERR: { errCode: userUpdateApiErrors_e.USER_UPDATE_API_PASSWORD_EASY, errMsg: props.ERR.error.message },
          };
        }
        if (
          props.ERR.error?.message == '[password:minLength:user:password] Value must be at least 8 characters long.'
        ) {
          return {
            ERR: { errCode: userUpdateApiErrors_e.USER_UPDATE_API_PASSWORD_TOO_SHORT, errMsg: props.ERR.error.message },
          };
        }
        if (
          props.ERR.error?.message == 'Variable "$id" of required type "ID!" was not provided.' ||
          props.ERR.error?.message ==
            'Argument passed in must be a single String of 12 bytes or a string of 24 hex characters'
        ) {
          return {
            ERR: {
              errCode: userUpdateApiErrors_e.USER_UPDATE_API_ID_INCORRECT_OR_NOT_PROVIDED,
              errMsg: props.ERR.error.message,
            },
          };
        }
