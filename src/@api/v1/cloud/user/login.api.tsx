...
export interface error_i {
  ...
  message?:
    | '[passwordAuth:secret:mismatch] The password provided is incorrect'
...
export const v1: (props: v1_props) => Promise<_loginAPi_returnType> = async ({ email, password, log }: v1_props) => {
    ...
    resolveData:
      ...
      (props) => {
        ...
        if (
          ...
          return {
            ERR: {
              errCode: loginApiErrors_e.LOGIN_API_UNKNOWN_EMAIL,
              errMsg: props.ERR.error.message + ' You can consider signingUp with the provided email',
            },
          };
        }
...
type useLogin_t = (props: {
  ...
  onLoginSuccess?: (props: { user: USER_t; devices: DEVICE_t[] }) => void;
...
export const useLogin: useLogin_t = async ({ email, password, ...props }) => {
  if (!email || !UNIVERSALS.util.ValidateEmail({ email })) {
    ...
    const tempError = {
      errCode: loginValidationError_e.LOGIN_VALIDATION_EMAIL_REQUIRED,
      errMsg: 'Kindly, provide a valid email address',
    };
  ...
  if (password && password.length < 8) {
    ...
    const tempError = {
      errCode: loginValidationError_e.LOGIN_VALIDATION_PASSWORD_REQUIRED,
      errMsg: 'Kindly, provide a valid password',
    };
    props.onValidateDataFailed ? props.onValidateDataFailed(tempError) : {};
    props.onLoginFailed ? props.onLoginFailed({ ERR: tempError }) : {};
    return;
  ...
  if (res.RES?.id) {
    ...
    const __t1_devices = UNIVERSALS.GLOBALS.convert_Devices_backendToLocal({
      devices: res.RES.devices ? res.RES.devices : [],
    });
