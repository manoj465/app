...
export const convertTimersStringToObj: convertTimersStringToObj_t = ({ timersString, log }) => {
  ...
  if (timersString) {
    try {
      let timersObject: (Omit<Omit<Omit<TIMER_t, 'DAYS'>, 'ET'>, 'DT'> & { DAYS: number; ET: number; DT: number })[] =
        JSON.parse(timersString);
      if (timersObject) {
        ...
        const newTimersObject = timersObject.map((timer, index) => {
          ...
          let tempDAYS: [boolean, boolean, boolean, boolean, boolean, boolean, boolean] = [
            false,
            false,
            false,
            false,
          ...
          let tempTimer: TIMER_t = {
            ...timer,
            ET: timer.ET == 1 ? TIMER_EVENT_TYPE_e.ON : TIMER_EVENT_TYPE_e.OFF,
            DT: timer.DT == 1 ? TIMER_DAYTIME_e.AM : TIMER_DAYTIME_e.PM,
            DAYS: tempDAYS,
          };
          return tempTimer;
        });
...
type converLocalTimerObjectToBackendString_t = (
  props: converLocalTimerObjectToBackendString_props
) => string | undefined;
export const converLocalTimerObjectToBackendString: converLocalTimerObjectToBackendString_t = ({ timers, log }) => {
  let newTimers = timers.map((timer) => {
    return {
      ...timer,
      ET: timer.ET == TIMER_EVENT_TYPE_e.ON ? 1 : timer.ET == TIMER_EVENT_TYPE_e.OFF ? 2 : 0,
      DT: timer.DT == TIMER_DAYTIME_e.AM ? 1 : timer.DT == TIMER_DAYTIME_e.PM ? 2 : 0,
      DAYS: (() => {
        let daysInt = 0;
