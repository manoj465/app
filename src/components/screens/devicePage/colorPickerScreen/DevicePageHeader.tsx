...
export const DevicePageHeader = ({ navigation, device: _device, headBackgroundColor, log }: Props) => {
  let device = useSelector<appState, DEVICE_t | undefined>((state) =>
    state.deviceReducer.deviceList.find((item) => item.Mac == _device.Mac)
  );
    ...
    >
      ...
      <View style={styles.nameContainer}>
        ...
        >
          <NewRectButtonWithChildren
            onPress={() => {
              appOperator.device({
                ...
                stateObject: {
                  state: device?.channel?.preState ? device.channel.preState : channelState_e.CH_STATE_1,
                },
          ...
          <Text style={styles.deviceName}>{device.deviceName ? device.deviceName : 'unnamed'}</Text>
        </View>
      </View>
