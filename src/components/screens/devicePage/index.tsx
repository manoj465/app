...
export default ({
  navigation,
  route: {
    params: { device: deviceFromProp },
  },
}: Props) => {
  const [screen, setScreen] = useState<'colorPicker' | 'timer' | 'deviceSetting'>('colorPicker');
  //@ts-ignore
  const device = useSelector<appState, DEVICE_t | undefined>((state) =>
    state.deviceReducer.deviceList.find((item) => item.Mac == deviceFromProp.Mac)
  );
