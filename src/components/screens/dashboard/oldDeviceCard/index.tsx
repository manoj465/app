...
import { deviceType_e, channelState_e } from '../../../../../sternet/helpers/universals/device/deviceEnum';
...
export const DeviceCard = ({ navigation, device, log, setToBeDeletedDevice }: props) => {
    ...
    <TouchableOpacity
      ...
      onPress={() => {
        if (
          ...
          device.channel.deviceType == deviceType_e.deviceType_RGBW
        ) {
    ...
    >
      <LinearGradient
        ...
        colors={(() => {
          ...
          if (
            (device.channel.deviceType == deviceType_e.deviceType_NW4 ||
              device.channel.deviceType == deviceType_e.deviceType_NW) &&
            //@ts-ignore
            device.channel.outputChannnel[0].temprature < 4000
          ) {
            return ['#ff0000', '#ff00ff'];
          } else if (device.channel.deviceType == deviceType_e.deviceType_RGB) {
            return [
              hsv2hex({
                hsv: [
                  //@ts-ignore
                  device.channel.outputChannnel[0].h,
                  //@ts-ignore
                  device.channel.outputChannnel[0].s > 60 ? device.channel.outputChannnel[0].s : 60,
                  100,
                ],
              }),
              hsv2hex({
                hsv: [
                  //@ts-ignore
                  device.channel.outputChannnel[0].h + 60,
                  //@ts-ignore
                  device.channel.outputChannnel[0].s > 60 ? device.channel.outputChannnel[0].s : 60,
                  100,
                ],
        ...
        >
          ...
          <NewRectButtonWithChildren /** device icon - turn on/off button */
            ...
            onPress={() => {
              appOperator.device({
                ...
                stateObject: {
                  state:
                    device.channel.state == channelState_e.CH_STATE_OFF
                      ? device.channel.preState
                        ? device.channel.preState
                        : channelState_e.CH_STATE_1
                      : channelState_e.CH_STATE_OFF,
                },
          ...
          >
            <MaterialCommunityIcons
              name={device.channel.state == channelState_e.CH_STATE_OFF ? 'lightbulb-off' : 'lightbulb-on-outline'}
              size={24}
              color="black"
            />
          </NewRectButtonWithChildren>

          <Text /**device name */ style={[styles.deviceName, { color: '#fff' }]}>
