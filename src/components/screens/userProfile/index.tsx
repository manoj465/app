...
export type navigationProp = StackNavigationProp<MainRouterStackParamList, 'user'>;
type routeProp = RouteProp<MainRouterStackParamList, 'user'>;

export enum userScreenState {
  USER_PROFILE,
  SIGNUP,
  UPDDATE,
  LOGIN,
...
const UserScreen = ({ navigation }: props) => {
  ...
  const [userPageView, setUserPageView] = useState<userScreenState>(userScreenState.LOGIN);

  if (userPageView == userScreenState.USER_PROFILE)
    return (
      <Container style={{ backgroundColor: '#fff', paddingHorizontal: 15 }}>
        ...
        >
          ...
          <NewRectButtonWithChildren /* Sec3: back button */
            ...
            onPress={() => {
              setUserPageView(userScreenState.UPDDATE);
            }}
          >
  ...
  return (
    <View style={[{}, style]}>
      ...
      <View style={{ borderWidth: 0.5, borderRadius: 10, borderColor: primaryColor }}>
        <TextInput
          style={{ height: 50, marginHorizontal: 10, fontSize: 15, color: primaryColor }}
