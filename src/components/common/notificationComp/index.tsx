...
export default (props: Props) => {
  let notifications = useSelector((state: appState) =>
    props.topic ? state.appCTXReducer.notifications.filter((item) => item.topic == props.topic) : state.appCTXReducer.notifications
  );
  ...
  return (
    ...
    >
      {notifications.map((notification, index) => {
        return <Card key={index} index={index} notification={notification} notifications={notifications} />;
      })}
    </View>
  );
};

const Card = (props: { notification: any; notifications: any; index: number }) => {
  ...
  let cardColor = props.notification.type == notificationType_e.ALERT ? '#e74c3c' : '#2dff70';
...
export const NOTIFY = (props: { topic: 'PAIRING' | 'UNIVERSAL'; title: String; subTitle?: String; color?: String; type: 'NORMAL' | 'ALERT' | 'ERROR' | 'SUCCESS' }) => {
  reduxStore.store.dispatch(
    reduxStore.actions.appCTX.notificationsRedux({
      newNotification: {
        topic: props.topic,
        ...
        color: props.color,
        type: props.type == 'ALERT' ? notificationType_e.ALERT : notificationType_e.NORMAL,
      },
    })
  );
};
