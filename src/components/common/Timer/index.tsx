...
const days = [{ day: 'M' }, { day: 'T' }, { day: 'W' }, { day: 'T' }, { day: 'F' }, { day: 'S' }, { day: 'S' }];
...
export const Timer = ({ device, log }: Props) => {
  const deviceFromStore = useSelector((state: appState) =>
    state.deviceReducer.deviceList.find((item) => item.Mac == device.Mac)
  );
  const [timerInEditor, setTimerInEditor] = useState<(Omit<TIMER_t, 'id'> & { id?: string }) | undefined>(undefined);
  ...
  return (
    ...
    >
      {deviceFromStore &&
        deviceFromStore.timers.map((timerFromProp, index) => {
          /* Sec2: timer block container */
          //console.log("timer from props = = = = = " + JSON.stringify(timerFromProp))
          return (
            <View style={styles.timerBlockConatiner} key={index}>
              ...
              >
                <View
                  style={{
                    width: 1,
                    borderLeftColor: timerFromProp.ET == TIMER_EVENT_TYPE_e.ON ? appPositiveColor : '#aaa',
                    borderLeftWidth: 2,
                  }}
                ></View>
                <View>
                  ...
                  >
                    ...
                    {timerFromProp.M + ' '}
                    {timerFromProp.DT == TIMER_DAYTIME_e.AM ? 'AM' : 'PM'})
                  </Text>
                  <Text style={{ marginLeft: 10 }}>
                    {timerFromProp.STATUS == TIMER_STATUS_e.INACTIVE ? 'snoozed' : 'Active'}
                  </Text>
                </View>
              </View>
              ...
              >
                ...
                >
                  <Ionicons
                    ...
                    color="white"
                    size={80}
                    style={{ position: 'absolute', right: '10%', top: '5%', opacity: 0.3 }}
                  />
                </LinearGradient>
                ...
                >
                  <View /* Sec5: Event Type */ style={{ marginLeft: 20, marginTop: 10 }}>
                    <Text style={{ color: 'white', fontSize: 12 }}>
                      {timerFromProp?.ET == TIMER_EVENT_TYPE_e.ON ? 'TURN ON' : 'TURN OFF'}
                    </Text>
                  </View>
                ...
                <View /* Sec4: Days Conatiner */ style={styles.weekDaysConatiner}>
                ...
                >
                  ...
                  <NewRectButtonWithChildren /* Sec5: DELETE BUTTON */
                    ...
                    onPress={() => {
                      ...
                      let newTimers: TIMER_t[] = [];
                      deviceFromStore.timers.forEach((deleteTimerObj, deleteTimerObjIndex) => {
                        if (deleteTimerObj.id == timerFromProp.id) {
                          _log?.print(
                            'timer to be deleted at index ' + deleteTimerObjIndex + ', timerId is ' + deleteTimerObj.id
                          );
                        } else newTimers.push(deleteTimerObj);
                      });
      ...
      {deviceFromStore?.timers.length == 0 && (
        <View style={[STYLES.center, { marginTop: 25 }]}>
          <Image style={{}} source={require('../../../../assets/images/devicePagesAssetes/noTimer.png')} />
        </View>
      )}

      {deviceFromStore && deviceFromStore.timers?.length < 5 && (
        <NewRectButtonWithChildren /* Sec2: AddNew Event Button */
          style={[
            {
              justifyContent: 'center',
              alignItems: 'center',
          ...
          onPress={() => {
            setTimerInEditor({
              ...
              H: 10,
              M: 0,
              ET: TIMER_EVENT_TYPE_e.ON,
              DT: TIMER_DAYTIME_e.AM,
              STATUS: 1,
            });
          }}
        >
          <Ionicons name="ios-add-circle" size={50} color="#555" />
          <Text style={{ position: 'absolute', bottom: 10, fontWeight: 'bold' }}>ADD NEW EVENT</Text>
        </NewRectButtonWithChildren>
      )}
